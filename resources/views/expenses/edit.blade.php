<x-layouts.app>
    ...
    <h1>Edit Expense</h1>
    <div class="bg-white rounded-lg p-4 gap-4 flex flex-col">
        <h1 class="font-bold">Expense Form</h1>
        <form action="{{ route('expenses.update', $expense) }}" method="POST" class="flex flex-col gap-4">
            @csrf
            {{-- @method('PUT') --}}
            {{-- <input name="title" placeholder="Title" value="{{ $expense->title }}" class="bg-gray-100 p-2 rounded-md"> --}}
            <x-forms.input label="Title" name="title" type="text" value="{{ $expense->title }}" autofocus />
            <input name="description" placeholder="Description" value="{{ $expense->description }}" class="bg-gray-100 p-2 rounded-md">
            <input type="number" step="0.01" name="amount" placeholder="Amount" value="{{ $expense->amount }}" class="bg-gray-100 p-2 rounded-md">
            <input name="category" placeholder="Category" value="{{ $expense->category }}" class="bg-gray-100 p-2 rounded-md">
            {{-- <input type="date" name="spent_at" value="{{ $expense->spent_at }}"> --}}
            <textarea name="notes" class="bg-gray-100 p-2 rounded-md">{{ $expense->notes }}</textarea>
